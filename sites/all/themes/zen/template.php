...
drupal_add_js(drupal_get_path('theme', 'zen') . '/zen-internals/js/custom.js', 'theme');
...
function zen_preprocess_page(&$vars, $hook) {
  ...
  //for orgnization edit and clone page
  if(arg(0) == 'node' && is_numeric(arg(1))){
	 foreach($vars['classes_array'] as $value){
	 	if($value == 'node-type-orgnization'){
	 		if( arg(2) == 'edit'){
	 		 	$vars['classes_array'][] = 'orgnization-edit';
	 		}
	 		else if( arg(2) == 'clone'){
	 			$vars['classes_array'][] = 'orgnization-clone';
	 		}
	 		else if( arg(2) == 'delete'){
	 			$vars['classes_array'][] = 'orgnization-delete';
	 		}
	 	}
	 }
  }
  
  //for orgnizations search page
  if(arg(0) == 'orgnizations' && arg(1) == 'search' && is_numeric(arg(2))){
  	$vars['classes_array'][] = 'page-orgnizations-search';
  }
  
  //for add grow experience page
  if(arg(0) == 'node' && arg(1) == 'add' && arg(2) == 'grow'){
  	$vars['classes_array'][] = 'grow-add';
  }
  
  //for orgnization edit page
  if(arg(0) == 'orgnization' && arg(1) == 'comment' && is_numeric(arg(2))){
	  $vars['classes_array'][] = 'page-orgnization-comment';
  }
  
  //for orgnization edit page
  if(arg(0) == 'register' && arg(1) == 'success' && is_numeric(arg(2))){
	  $vars['classes_array'][] = 'page-register-success';
  }
  
  //for comment edit page
  if(arg(0) == 'comment' && arg(1) == 'edit' && is_numeric(arg(2))){
	  $vars['classes_array'][] = 'comment-edit';
  }
  
  //for daily virtue edit page
  foreach($vars['classes_array'] as $value){
 	 if($value == 'node-type-daily-virtue'){
 		 $vars['classes_array'][] = 'page-node-add-daily-virtue';
 	 }
  }
  
  //for all the pages
  $vars['classes_array'][] = 'global';	

  if($vars["node"]->type == 'aeb_virtue'){
  	$vars['template_files'][0] = 'page-virtuenode';
  }
}
...
function zen_preprocess_node(&$vars, $hook) {
  ...
  //获取第一张图片node
  if($vars['type'] == 'node_gallery_gallery'){
  	
  	$sql = "SELECT ngi.nid AS nid, ngi.weight AS weight FROM node n LEFT JOIN node_gallery_images ngi ON n.nid = ngi.gid
			 WHERE (n.status <> 0) AND (ngi.gid = %d)
			 ORDER BY weight ASC, nid DESC limit 0,1";
  	$result = db_query($sql, $vars['nid']);
  	if($row = db_fetch_object($result)){
  		   $node = node_load($row -> nid);
  		   	 //dpm($node);
  		   $img_navigator = theme('node_gallery_image_navigator', node_gallery_get_image_navigator($node->gid, $node->nid), $node);
		   //$node = node_build_content($node, $teaser, $page);	
		  	  
		   $image = theme('imagecache', 'node-gallery-display', $node -> field_node_gallery_image[0]['filepath']);	
		   
		   $vars['firstImage'] = '<div id="bigImgage" class="bigimg">
		   							<table width="512px" height="445px" style="margin:0 0 0 5px;"><tr><td>'.$image.'</td></tr></table>'.$img_navigator.'		
								    </div>
								    <div id="image-dct" class="img-description">
								    '.$node -> body.'
								    </div>';
  	}
  }
  ...
  if($vars['type'] == 'aeb_virtue'){
  	...
  	inner join {users} u on n.uid = u.uid
  	 where ctdv.field_aeb_virtue_nid = ".$vars['nid']." order by n.created desc";
